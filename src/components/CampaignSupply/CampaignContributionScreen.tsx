...
import { BottomCard, Button, Icon, IconName } from '~components/common'
import { useId } from '~components/common/useId'

export const CampaignContributionScreen: React.FC = () => {
  const id = useId()
  const { data, loading } = useQuery<{ campaignContribution: any }>(
    gql`
  ...
  const { campaignContribution: { campaign, campaignContributionSupplies } } = data!
  const name = 'blah'
  ...
  return (
    <div className='px-6 bg-grey-100 h-screen'>
      ...
      {campaignContributionSupplies!.map(({ id, quantity, campaignSupply }) => (
        <div key={id!} className='flex border-b border-grey-200 items-center py-6 items-center'>
          <Icon icon={campaignSupply!.supply!.icon! as IconName} w={10} />
          <div className='flex-grow ml-4'>{campaign!.corporation!.name}</div>
          <div>{quantity}</div>
        </div>))}
